...
class ComposedStateProviderTest {
    ...
    @Test
    fun shouldProvideInitialStateFromStateProvider() =
        runTest {
    ...
    @Test
    fun shouldComposeWithFlowStateProviders() =
        runTest {
    ...
    @Test
    fun shouldSupportDelegationPattern() =
        runTest {
            ...
            val composer: ComposedStateProvider<TestState> =
                composedStateProvider(
    ...
    @Test
    fun shouldSupportEmptyComposer() =
        runTest {
            val composer =
                composedStateProvider(
    ...
    @Test
    fun shouldComposeWithStateContainers() =
        runTest {
    ...
    @Test
    fun shouldComposeWithStateHolders() =
        runTest {
    ...
    @Test
    fun shouldCombineMultipleSourceTypes() =
        runTest {
    ...
    @Test
    fun shouldComposeUsingExtensionFunction() =
        runTest {
            val nameFlow = MutableStateFlow("extension")

            val composer =
                composedStateProvider(
                    initialState = TestState(count = 50),
                    composer = {
                        nameFlow into { copy(name = it) }
                    },
                )

            val container = stateContainer(TestState(count = 50))
            container.compose(backgroundScope) { with(composer) { compose() } }

            container.state.test {
                awaitItem() shouldBe TestState(count = 50)
                awaitItem() shouldBe TestState(name = "extension", count = 50)

                nameFlow.value = "updated"
                awaitItem() shouldBe TestState(name = "updated", count = 50)
